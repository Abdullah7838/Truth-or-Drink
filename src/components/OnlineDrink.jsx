...
function OnlineDrink({ myusername }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      axios
        .get(
          `https://truth-or-dare-backend-1kg2.onrender.com/drink/drink/api/rooms/${roomId}`
        )
        .then((res) => {
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const sendHI = async () => {
      ...
      try {
        await axios.post(
          `https://truth-or-dare-backend-1kg2.onrender.com/drink/drink/api/${roomId}/ping`,
          {
            username: myusername,
            message: msg,
          }
    ...
    sendHI();
    const interval = setInterval(sendHI, 10000);
        ...
        {currentPrompt.text && (
          <div className="bg-white/90 border-l-4 border-purple-500 shadow-md p-5 rounded-2xl mb-4 text-center transition-all duration-500 animate-fade-in">
            <p className="text-lg font-bold text-purple-700">
              🧠 {currentUser} must do this {currentPrompt.type.toUpperCase()}:
            </p>
